...
void make_action(const TypeVolt* Volt){
	...
	switch(ActiveGas){
      case(LEFT):
        ...
        if((Volt->PressLeft <= SWTCH_threshold) && 
					(BARANKI || Alarm.LineMin) &&
					(Volt->PressRight >= SWTCH_threshold)){
						ActiveGas = RIGHT;
       ...
       case(RIGHT):
        ...
        HAL_GPIO_WritePin(GPIOB, LED_left_gas, GPIO_PIN_RESET);
        ...
        if((Volt->PressRight <= SWTCH_threshold) &&
					(BARANKI || Alarm.LineMin) &&
					(Volt->PressLeft >= SWTCH_threshold)){
						ActiveGas = LEFT;
						ActiveCylinder = LEFT;
        }
        if((ConcOldOK == 0)&&(!Alarm.ConcentratorNOT_OK)){
    ...
    if(OneSeconTick){
			if(ButtAlCounter == BUTT_TRIM){
				...
				LoadLEDs((uint32_t)ALRM_BYTE | ((uint32_t)RSSI_WORD)<<10);
			}else{
					if(flagLEDodd){
						LEDpercent(LEDvalueL/2, LEDvalueR/2);
						flagLEDodd = 0;
					}else{
						LEDpercent(LEDvalueL/2 + LEDvalueL%2, LEDvalueR/2 + LEDvalueR%2);
						flagLEDodd = 1;
					}   											
			}		
		OneSeconTick = 0;
    ...
    if(((Volt->PressLeft <= SWTCH_threshold)&&(Volt->PressRight <= ALRM_threshold))
       ||((Volt->PressRight <= SWTCH_threshold)&&(Volt->PressLeft <= ALRM_threshold))){
   ...
   //Line is high
    ...
    if(HAL_GPIO_ReadPin(GPIOB, SMC_C_MAX)){
      Alarm.ConcentratorMax = 1;
    }else{
      Alarm.ConcentratorMax = 0;
    }
    //Concentrator pressure low
    if(HAL_GPIO_ReadPin(GPIOB, SMC_C_OK)){
      Alarm.ConcentratorNOT_OK = 0;
    }else{
    ...
    if(Alarm.BatteryOut || (NoLinkCounter > (MAX_NOLINK / 2))){
